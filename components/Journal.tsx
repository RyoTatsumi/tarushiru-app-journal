...
export const Journal: React.FC<JournalProps> = ({ entries, onAddEntry, onUpdateEntry, onDeleteEntry, profile }) => {
  ...
  const handleSubmit = async () => {
    ...
    setIsAnalyzing(true);
    try {
      // 過去の直近エントリー（最大5件）を取得して文脈として渡す
      const recentEntries = entries.slice(-5);
      
      const analysis = await analyzeJournalEntry(content, profile, recentEntries);
          ...
          <div className="space-y-4">
            ...
            <div key={entry.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 group">
                ...
                <div className="bg-navy-50 rounded-xl p-4 space-y-4 border border-navy-100 relative overflow-hidden">
                    {/* Decorative Background Icon for AI personalization feedback */}
                    ...
                    {entry.aiComment && (
                        <div className="flex space-x-3 mb-2 relative z-10">
                            ...
                            <p className="text-sm text-navy-800 font-medium italic leading-relaxed">
                                {entry.aiComment}
                            </p>
                        </div>
                    )}
